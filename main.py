# Program Name: Stock Predictor
# Programmer Name: Vyom Kapadia, Anikait Choudury, Rag Patel
# Date of Completion: June 09, 2024
# Description: This program (in combination with the index.html, styles.css, and script.js files)
#              asks the user for a stock name and creates an machine learning model for the stock
#              and predicts the last few days of values of that stock. To run this app, run the python
#              file and double click on the index.html file in the file explorer to open a web page in 
#              chrome. After doing these 2 things, you can follow the web page (by entering a stock name).
#              Make sure to have all the libraries downloaded.
...
# To download the libraries, do "pip install {library name}" for each library so numpy,
# pandas, tensorflow, sklearn, fastapi, pydantic, and uvicorn.
import numpy as np
import pandas as pd
...
def get_train_test_data(FILE_NAME, dataset_indicator = "close"):
    ...
    train_data = scaled_data[len(scaled_data)-train_data_len:, :]
    train_data = train_data[::-1]
    x_train, y_train = [], []
    
    test_data = scaled_data[:len(scaled_data)-train_data_len + SEQ_LEN, :]
